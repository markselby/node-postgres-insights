...
p.columns = function columns(table, database, schema) {
  var q = queries.columns(table, database || this.database, schema);
  return q;
};

p.indexes = function indexes(table, database, schema) {
  var q = queries.indexes(table, database || this.database, schema);
  return q;
};

p.tableScans = function indexStats(schema) {
  var q = queries.tableScans(schema);
  return q;
};

p.indexStats = function indexStats(schema) {
  var q = queries.indexStats(this.database, schema);
  return q;
};

p.foreignKeys = function foreignKeys() {
  var q = queries.foreignKeys();
  return q;
};
