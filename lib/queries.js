...
exports.columns = function columns(table, database, schema) {
  ...
  q.order('table_name, ordinal_position');
  q.limit(null);
  return q;
};
...
exports.tables = function tables(database, schema) {
  ...
  q.select('COALESCE((SELECT sum(avg_width) AS average_row_size FROM pg_stats WHERE tablename=table_name), 0) AS average_row_size');
...
exports.tableScans = function tables(schema) {
  schema = schema || 'public';
  var q = new Sql();
  q.select('relname AS table_name, pg_relation_size(relname::regclass) AS table_size, seq_scan, idx_scan');
  q.from('pg_stat_all_tables');
  q.param(schema);
  q.where('schemaname = ' + q.paramNo());
  q.order('relname DESC');
  q.limit(undefined);
  return q;
};
